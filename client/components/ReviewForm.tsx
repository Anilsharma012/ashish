...
export default function ReviewForm({ targetId, targetType = "property" }: { targetId: string; targetType?: string }) {
  ...
  if (!token) {
    return (
      <div className="mt-4 text-sm">
        ...
        <Link to="/user-login" className="text-[#C70000] hover:underline">Login</Link>
      </div>
    );
  }
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      const res = await submitReview({ targetId, targetType, rating, title: title.trim() || undefined, comment: comment.trim() });
      if (res.ok) {
        toast({ title: "Submitted for approval", description: "Your review will appear after approval." });
        setRating(0);
        setTitle("");
        setComment("");
  ...
  return (
    <div className="mt-4">
      {pendingNote && <div className="text-xs text-gray-500 mb-2" aria-live="polite">{pendingNote}</div>}
      <form onSubmit={onSubmit} className="rounded-xl shadow-sm border p-4 bg-white">
        ...
        <label className="block text-sm font-medium mb-1" htmlFor="title">Title (optional)</label>
        <input
          id="title"
          value={title}
